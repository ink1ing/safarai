...
function extractVideoContext(win, doc, site) {
  ...
  return {
    metadata: {
      ...
      videoContentBasis: "page_metadata",
    },
  };
}
...
function findPrimaryVideo(win, doc) {
  ...
  for (const video of videos) {
    if (!isNodeVisible(win, video)) {
      continue;
    }

    const rect = getNodeRect(video);
    const width = normalizeVideoNumber(rect?.width) ?? normalizeVideoNumber(video.videoWidth) ?? 0;
    const height = normalizeVideoNumber(rect?.height) ?? normalizeVideoNumber(video.videoHeight) ?? 0;
