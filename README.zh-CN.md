...
针对视频页面，项目会结合以下信息增强提示词：

- 平台字幕抽取结果
- 页面 metadata 中的视频状态信息
- 通过 Safari 消息触发的视频帧慢速采样结果

这使得它在 YouTube、Bilibili 一类视频页上，不仅能依赖字幕，还能在字幕不足时补充视觉线索。
...
- 敏感设置会写入共享容器中的私有文件
- OpenAI 兼容接口的凭据存储在本地
- 浏览器页面上下文可能包含选中文本、正文文本、字幕文本和视频相关 metadata
